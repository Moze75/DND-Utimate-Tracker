...
export function InventoryItemEditModal({
  ...
  const [quantity, setQuantity] = React.useState<number>(meta.quantity ?? 1);
  const [wDice, setWDice] = React.useState(meta.weapon?.damageDice || '1d6');
  const [wType, setWType] = React.useState<'Tranchant' | 'Perforant' | 'Contondant'>(meta.weapon?.damageType || 'Tranchant');
  const [wProps, setWProps] = React.useState(meta.weapon?.properties || '');
  const [wRange, setWRange] = React.useState(meta.weapon?.range || 'Corps à corps');
  ...
  const save = async () => {
    try {
      ...
      if (nextMeta.type === 'weapon') {
        nextMeta = { ...nextMeta, weapon: { damageDice: wDice || '1d6', damageType: wType, properties: wProps || '', range: wRange || 'Corps à corps' } };
      }
